<div class="w-full px-4">
    ...
    <div class="flex space-x-3 overflow-x-auto scrollbar-hide p-4 bg-white rounded-xl shadow-md">
        @forelse($stories as $index => $story)
            <div wire:click="openStory({{ $index }})"
                ...
                <div class="relative">
                    <!-- Story Image -->
                    <div class="w-20 h-20 rounded-full bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 p-0.5">
    ...
    <!-- Story Viewer Modal -->
    @if ($selectedStory)
        <div class="fixed inset-0 bg-black bg-opacity-75 z-50 flex items-center justify-center" wire:click="closeStory">
            <div class="relative max-w-md w-full mx-4" wire:click.stop>
                ...
                <div class="absolute top-0 left-0 right-0 z-10 p-4">
                    ...
                    <div class="flex items-center justify-between text-white">
                        <div class="flex items-center space-x-3">
                            <div class="w-8 h-8 rounded-full bg-gradient-to-r from-purple-400 to-pink-500 p-0.5">
                                <div class="w-full h-full rounded-full bg-white p-0.5">
                ...
                <div class="relative bg-black rounded-lg overflow-hidden aspect-[9/16] max-h-[70vh]">
                    ...
                    <!-- Navigation Areas -->
                    @if ($currentIndex > 0)
                        <div wire:click="prevStory" class="absolute left-0 top-0 w-1/3 h-full cursor-pointer z-20">
                        </div>
                    @endif

                    @if ($currentIndex < count($stories) - 1)
                        <div wire:click="nextStory" class="absolute right-0 top-0 w-1/3 h-full cursor-pointer z-20">
                        </div>
                    @endif
                </div>
